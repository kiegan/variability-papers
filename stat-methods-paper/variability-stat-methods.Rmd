...
## Complex Data Structures \label{complex-data-structures}  
...
\hh{XXX include references to each section with ref and label.} \kr{done, see below}
...
Of primary importance in measurement systems analysis are the estimates of variance components associated with each random effect. For example, the estimated value of $\sigma^2_d$, $\hat{\sigma}^2_d$, estimates the variance in the measurement process associated with differences across measurement devices. We also consider two summary values:  \hh{XXX I don't much like the long subscripts of repeatability and reproducibility. At a minimum they should be encapsulated in the command 'text'. Are you actually using these sigmas? if not, just write it out as actual text (keep the equation and the numbers): repeatability is reflected by: XXX reproducibility is reflected by: YYY} \kr{We report these sigams in a table at the end, but how about shortening to "repeat" and "reprod" as text, as below? can update table too if we like this.}
%
\begin{align}
...
calculate a bullet profile by aggregating height measurements along a small band (marked in light red) across the bullet surface. The resulting height measurement is denoted as  $\bar{z}_{i\cdot}$ for each location $i$ along $x$.
...
The resulting LEA signatures are a sequence of residual values \kr{$e_i = \bar{z}_{i\cdot} - \hat{z}_{i\cdot}$}, \hh{XXX usually residuals are defined as observed minus expected. I don't think it matters, but it should be fixed.} where $\hat{z}_{i\cdot}$ are predicted height values for location $x_i$ from a LOESS model fit to the bullet curvature. 
The series of relative height measurements and their corresponding locations $x_i$ are the structure of interest, so for clarity we will denote LEA signatures as a set of measurements $(x_i, z_i)$, \kr{replacing $\bar{z}_{i\cdot}$ with simply $z_i$ - is this better?}.\hh{XXX I'm not sure you mean $z_i$. But I'd be in favor of renaming the $\bar{z}_{i\cdot}$ to $z_i$.}, $i = 1, \dots, \ell$ where $\ell$ is the number of $x$ locations on a signature for LEA $L$. \autoref{sig-structure} depicts 228 signatures corresponding to scans from three bullets  of barrel 6 in Hamby set 224. Scans were taken by multiple operators on two different microscopes. These repetitions are shown in grey, and demonstrate the variability within the measurement process of translating striation patterns on bullets to signature data objects. One of these signature is singled out and shown in dark green to emphasize the general structure. 
\hh{XXX It looks like the  signatures are also zero-centered. That needs mentioning} \kr{The signatures depicted in \autoref{sig-structure} have been coaligned in the $x$ direction using maximum cross-correlation function. In addition, trailing edges of each signature were trimmed to balance $x$ location indices for modeling. XXX something like this? I don't believe these are zero centered, these are just signatures as-is.}  
...
\kr{We define a model to estimate measurement variability of LEA signatures in the following way. Let $z_{Lijkmn}$ be the measured signature height value for land $L = 1, \dots, 6$ at location $x_i: i = 1, 2, \dots, \ell$, on part $j$, scanned by operator $k$, on device $m$, and at repetition $n$.  
...
\kr{Note that Equation \ref{sig-model} contains a more complex fixed effect structure than the traditional model defined in Equation \ref{model1}. We define a fixed effect $\mu_{Li}$ indexed by both land $L$ and location $i$ in order to separate structural variability in the signature from measurement variability, our mechanism of interest. LEA signature data contain an inherently complex structure of center, as signatures are made up of series of peaks and valleys which act as the signal of interest for evidence comparison. We are interested in measuring the variability around that signal measurement rather than variability of the signal itself, and therefore must account for differences in structural height by location as part of our fixed effects structure. XXX better?}
...
Despite this adjustment, there is still significant structure remaining in the LEA signature data which may \kr{lead to systematic underestimation of quantities of interest when left in the model,} \hh{XXX 'cloud' is a bit vague. Be more specific. The structure you are referring to is autocorrelation and it leads to a systematic underestimation of the variance components when left in the model.} such as the variance components associated with operator and device. Our approach to mitigating this misrepresentation, as shown in Equation \ref{sig-model}, is to carefully define the grouping structure used to estimate variance components. \kr{We define random effect grouping structure by both study factor and location $Li$, practically accomplished by interacting location with study factor in each random effect}. \kr{Consider} for example the variability of measurements across devices. It is of interest whether one device captures more extreme values, such as higher peak measurements and lower valley measurements. If we defined a random effect for device and grouped by device alone, without accounting for structural differences by location, extreme highs and extreme lows would average out across the entire group and the variability would be artificially deflated. \kr{XXXThis will go away: We address this by defining our random effect groups by both study factor and location $Li$. This is accomplished by interacting location with study factor in a random effect.}  
...
\hh{XXX Move the model equation up to the start of section 2.1. You don't have to be specific about the locations at that point. Once that you have addressed autocorrelation you can specify the location index in more detail.} \kr{done!}

\kr{The model is used as stated in Equation \ref{sig-model}, but using data at subsampled indices $x_i:i = 1, 101, \dots, 1+100*c$. XXX the rest of this can go away if we like the new framing:  
...
\begin{equation}
z_{Lijkmn} = \mu_{Li} + p_{Lij} + o_{Lik} + d_{Lim} + po_{Lijk} + pd_{Lijm} + od_{Likm} + pod_{Lijkm} + \epsilon_{Lijkmn}, \label{sig-model-old}
\end{equation}
...
# Data Collect\hh{ion} \label{data-collection}  
