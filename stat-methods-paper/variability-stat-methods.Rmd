...
where $\mu$ is a fixed, unknown measurement mean and all other model components are random effects \citep{Montgomery}. Assume \hh{all effects}  $p_j$, $o_k$, $d_m$, $po_{jk}$, $pd_{jm}$, $od_{km}$, and $pod_{jkm}$ are independent and identically distributed random variables which follow normal distributions centered at zero but \hh{have}  different variances. \hh{To clarify the connection, we index a variance by its corresponding random variable}; for example, $p_j \stackrel{iid}{\sim} N(0, \sigma^2_{p})$. We also assume $\epsilon_{jkmn} \stackrel{iid}{\sim} N(0, \sigma^2)$.  

Of primary importance in measurement systems analysis are the estimates of variance components associated with each random effect. For example, the estimated value of $\sigma^2_d$, $\hat{\sigma}^2_d$, estimates the variance in the measurement process associated with differences across measurement devices. We \hh{also} consider two summary values:  \hh{XXX I don't much like the long subscripts of repeatability and reproducibility. At a minimum they should be inencapsulated in the command text. Are you actually using these sigmas? if not, just write it out as actual text (keep the equation and the numbers): repeatability is reflected by: XXX reproducibility is reflected by: YYY}
%
\begin{align}
...
The series of relative height measurements and their corresponding locations $x_i$ are the structure of interest, so for clarity we will denote LEA signatures as a set of measurements $(x_i, z_i)$ \hh{XXX I'm not sure you mean $z_i$. But I'd be in favor of renaming the $\bar{z}_{i\cdot}$ to $z_i$.}, $i = 1, \dots, \ell$ where $\ell$ is the number of $x$ locations on a signature \hh{for LEA} $L$. \autoref{sig-structure} depicts \hh{231 signatures corresponding to three bullets  from Hamby set 224. All three bullets were fired through barrel 6. Scans were taken by multiple operators on two different microscopes.} These repetitions are shown in grey, and demonstrate variability within the measurement process of translating striation patterns on bullets to signature data objects. \hh{One of these signature is singled out and shown  in darkgreen to emphasize the general structure.}
...
The three-factor model defined in Equation \ref{model1} defines all effects as random save for a single fixed effect: a fixed,  measurement average, $\mu$ \hh{for each location}. LEA signature data inherently contain a more complex structure of center, as signatures are not made up of a univariate response corresponding to a single measurement value. \hh{XXX I'm not sure what you are trying to say in the next sentence.} Failing to account for differences in structural height by location will result in mischaracterization of variance components within the model; variability estimates will include the object's structural variability rather than measurement variability. To address this, we define the model's fixed effect structure by location $x_i$ on each land $L$: $\mu_{Li}$. 

\hh{XXX This seems in direct contradiction to the statement above. I think I'm missing something here. }
